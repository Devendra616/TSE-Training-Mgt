...
import { getAllUsers, createUser, updateUser, deleteUser, resetUserPassword, type CreateUserData } from '../services/users';
import { Card, CardContent, CardHeader, CardTitle } from '../components/ui/Card';
import { Input } from '../components/ui/Input';
import { Button } from '../components/ui/Button';
import { User, Shield, Moon, Sun, Lock, Users, Plus, Edit2, Trash2, KeyRound, Search, X } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Input label="Role" value={user?.role?.replace('_', ' ')} readonly className="capitalize" />
          <Input label="Employee ID" value={user?.id?.toString() || ''} readonly />
        </div>
        <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-200 rounded-lg text-sm">
          Note: Details are managed by the administrator. Contact support for changes.
        </div>
      </CardContent>
    </Card>
  );
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          <h3 className="font-medium text-gray-900 dark:text-white px-1">Theme Color</h3>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
            {THEME_COLORS.map((color) => (
              ...
              >
                ...
                <span className={cn(
                    "text-sm font-medium",
                    primaryColor === color.value ? "text-gray-900 dark:text-white" : "text-gray-600 dark:text-gray-400"
                )}>
                  {color.name}
                </span>
              </button>
    ...
    <div className="space-y-6">
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex justify-end gap-2">
                           <button
                             ...
                             onClick={() => { setSelectedUserForReset(user); setShowResetModal(true); }}
      ...
      {showResetModal && selectedUserForReset && (
        <PasswordResetModal
          ...
          onSubmit={(password) => resetPasswordMutation.mutate({ id: selectedUserForReset.id, password })}
