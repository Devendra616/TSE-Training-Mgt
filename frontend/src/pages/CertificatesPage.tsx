...
export function CertificatesPage() {
  ...
  const { data: certificates, isLoading } = useQuery({
    ...
    queryFn: () =>
      getCertificates(statusFilter ? { status: statusFilter } : undefined),
  });
  ...
  const selectAllDrafts = () => {
    const draftIds =
      certificates
        ?.filter((c) => c.workflowStatus === "draft")
        .map((c) => c.id) || [];
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {Object.entries(STATUS_CONFIG).map(([status, config]) => (
              <button
                key={status}
                onClick={() => {
                  setStatusFilter(status as WorkflowStatus);
                  setSelectedIds([]);
                }}
                className={cn(
                  "px-4 py-2 rounded-lg text-sm font-medium transition-colors",
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div>
                            ...
                            <div className="text-xs text-gray-500">
                              {formatSapIdWithToken(cert.employee)}
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                        ...
                        <td className="py-3 px-4 text-gray-600 dark:text-gray-400">
                          {cert.validFrom && cert.validUntil ? (
                            <>
                              {format(new Date(cert.validFrom), "MMM d, yyyy")}{" "}
                              -<br />
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  link.download =
                                    filename ||
                                    `${cert.certificateNumber || `certificate-${cert.id}`}.pdf`;
