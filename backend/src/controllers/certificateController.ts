      ...
      include: [
        {
          model: Training,
          as: "training",
          attributes: ["id", "name", "code", "trainingType"],
        },
        {
          model: Employee,
          as: "employee",
        ...
        },
        {
          model: Batch,
          as: "batch",
          attributes: ["id", "startDate", "endDate", "venue"],
        },
      ],
    ...
    const certificates = await Certificate.findAll({
      ...
      include: [
        ...
        },
        {
          model: Batch,
          as: "batch",
          attributes: ["id", "startDate", "endDate", "venue", "instructorName"],
        },
      ],
...
export const downloadCertificatePdf = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      // Try to find certificate record first
      const certificate = await Certificate.findByPk(certificateId);
      if (!certificate) {
        throw new NotFoundError("Certificate");
      }

      // If migrated certificate has an uploaded file reference, attempt to serve that file first.
      const uploadsDir = config.upload.certificatesDir;

      const getPreferredDownloadName = (certificate: any, fallback: string) => {
        if (!certificate.notes) return fallback;
        const parts = (certificate.notes as string)
          .split("|")
          .map((s) => s.trim())
          .filter(Boolean);
        const lastPart = parts[parts.length - 1];
        if (!lastPart) return fallback;
        // Use the last note part if it looks like a filename with extension.
        return /[^\\/]+\.[A-Za-z0-9]{1,10}$/.test(lastPart)
          ? lastPart
          : fallback;
      };

      const trySendUploadedFile = (
        filename?: string,
        downloadName?: string,
      ) => {
        if (!filename) return false;

        // Resolve both the basename and the raw value to support stored filenames
        // or paths returned from earlier migration records.
        ...
        for (const filePath of candidatePaths) {
          if (fs.existsSync(filePath)) {
            res.download(filePath, downloadName || path.basename(filePath));
            return true;
          }
        }
      ...
      const preferredDownloadName = getPreferredDownloadName(
        certificate,
        `certificate-${certificateId}.pdf`,
      );

      if (certificate.certificatePath) {
        const cp = certificate.certificatePath as string;
        const candidate = path.basename(cp);
        if (trySendUploadedFile(candidate, preferredDownloadName)) return;
      }
      ...
      if (certificate.notes) {
        ...
        if (trySendUploadedFile(last, preferredDownloadName)) return;
        for (const p of parts) {
          if (
            /\.pdf$/i.test(p) &&
            trySendUploadedFile(p, preferredDownloadName)
          )
            return;
        }
      }
      ...
      if (certificate.isMigrated) {
        throw new NotFoundError(
          "Original uploaded certificate file could not be located",
        );
      }
      ...
      console.error("downloadCertificatePdf failed:", err);
      throw err;
    }
  },
);
 ...
 */
export const generateCertificates = asyncHandler(
  async (req: Request, res: Response) => {
 ...
 */
export const submitForApproval = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
 ...
 */
export const bulkSubmitForApproval = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    logUserAction(req.user!.id, "bulk_submit_for_approval", "certificates", 0, {
      count: result[0],
    });
 ...
 */
export const approveCertificate = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    const certNumber = await CertSequence.getNextSequence(
    ...
    if (certificate.draftCreatedBy) {
      const employeeName =
        (certificate as any).employee?.fullName || "Employee";
      const trainingName = (certificate as any).training?.name || "Training";
...
export const bulkApprove = asyncHandler(async (req: Request, res: Response) => {
  ...
  logUserAction(req.user!.id, "bulk_approve", "certificates", 0, {
 ...
 */
export const rejectCertificate = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
 ...
 */
export const resubmitCertificate = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params;
    ...
    if (certificate.workflowStatus !== WorkflowStatus.REJECTED) {
      throw new ValidationError(
        "Only rejected certificates can be resubmitted",
      );
    }
