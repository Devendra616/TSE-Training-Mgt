...
import { Batch, Training, BatchEmployee, Employee, Attendance, BatchStatus } from '../models/index.js';
...
export const getAllBatches = asyncHandler(async (req: Request, res: Response) => {
  ...
  const batches = await Batch.findAll({
    where,
    include: [
      { model: Training, as: 'training', attributes: ['id', 'name', 'code', 'durationDays'] },
    ],
  ...
  const enrollmentCounts = await BatchEmployee.findAll({
    ...
    attributes: ['batchId', [sequelize.fn('COUNT', sequelize.col('id')), 'count']],
    group: ['batchId'],
    raw: true,
  }) as any[];
...
export const createBatch = asyncHandler(async (req: Request, res: Response) => {
  const { trainingId, startDate, endDate, capacity, venue, instructorName, notes } = req.body;
...
export const updateBatch = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const { startDate, endDate, capacity, venue, instructorName, status, notes } = req.body;
...
export const deleteBatch = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (enrollmentCount > 0) {
    throw new ValidationError('Cannot delete batch with enrolled employees. Remove enrollments first.');
  }
 ...
 */
export const enrollEmployees = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  if (currentCount + employeeIds.length > batch.capacity) {
    throw new ValidationError(`Cannot enroll ${employeeIds.length} employees. Batch capacity is ${batch.capacity}, current enrollment is ${currentCount}`);
  }
  ...
  const newEnrollments = employeeIds.filter((empId: number) => !alreadyEnrolled.includes(empId));
  ...
  logUserAction(req.user!.id, 'enroll', 'batches', batch.id, { employeeIds: newEnrollments });
...
export const removeEmployee = asyncHandler(async (req: Request, res: Response) => {
  ...
  logUserAction(req.user!.id, 'unenroll', 'batches', parseInt(id), { employeeId: parseInt(employeeId) });
...
export const getBatchAttendance = asyncHandler(async (req: Request, res: Response) => {
  ...
  const attendanceMatrix = enrollments.map((enrollment) => {
    const empAttendance = attendance.filter(a => a.employeeId === enrollment.employeeId);
    ...
    return {
      ...
      isComplete: empAttendance.filter(a => a.isPresent).length === durationDays,
    };
  });
...
export const markAttendance = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (dayNumber < 1 || dayNumber > durationDays) {
    throw new ValidationError(`Day number must be between 1 and ${durationDays}`);
  }
  ...
  logUserAction(req.user!.id, 'mark_attendance', 'batches', parseInt(id), { employeeId, dayNumber, isPresent });
...
export const bulkMarkAttendance = asyncHandler(async (req: Request, res: Response) => {
  ...
  const results = await Promise.all(
    records.map(async (record: { employeeId: number; isPresent: boolean }) => {
  ...
  logUserAction(req.user!.id, 'bulk_mark_attendance', 'batches', parseInt(id), { dayNumber, count: records.length });
...
export const cloneBatch = asyncHandler(async (req: Request, res: Response) => {
  ...
  logUserAction(req.user!.id, 'clone', 'batches', newBatch.id, { clonedFrom: originalBatch.id });
