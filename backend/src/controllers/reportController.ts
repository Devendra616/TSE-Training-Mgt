...
import { Certificate, Employee, Training, Batch, Department, BatchEmployee, WorkflowStatus } from '../models/index.js';
...
export const getBatchReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  const certifiedCount = certificates.filter((c) => c.workflowStatus === WorkflowStatus.APPROVED).length;
  const pendingCount = certificates.filter((c) => c.workflowStatus === WorkflowStatus.PENDING_APPROVAL).length;
  ...
  res.json({
    ...
    data: {
      ...
      stats: {
        ...
        completion: enrolledCount > 0 ? Math.round((certifiedCount / enrolledCount) * 100) : 0,
      },
      certificates,
    },
  });
 ...
 */
export const getTrainingReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  const currentlyValid = certificates.filter((c) => new Date(c.validUntil) >= new Date()).length;
...
export const getDepartmentReport = asyncHandler(async (req: Request, res: Response) => {
  ...
  const compliantCount = complianceData.filter((c) => c?.overallStatus === 'compliant').length;
  const dueSoonCount = complianceData.filter((c) => c?.overallStatus === 'due_soon').length;
  const overdueCount = complianceData.filter((c) => c?.overallStatus === 'overdue').length;
 ...
 */
export const exportCertificates = asyncHandler(async (req: Request, res: Response) => {
  ...
  const rows = certificates.map((cert) => [
    ...
    format(new Date(cert.validFrom), 'yyyy-MM-dd'),
